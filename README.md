...
The embedded config page is an HTML fragment whose root element has **id="configPage"** and **data-role="page"** (required by the Jellyfin web client’s `loadView`). It contains an iframe that loads the full settings from `/Plugins/PosterTags/ConfigurationPage`. Open **Dashboard → Plugins → Poster Tags → Settings** to use it.
